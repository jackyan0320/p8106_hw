...
|LDA         |                          0.557| 
|QDA         |                          0.529|
|KNN         | `r round(roc_knn$auc[[1]], 3)`| 

If we train the models using data from 1990 to 2008 and predict `direction` from 2009 to 2010, LDA has the largest AUC, thus generates the best prediction. KNN has the lowest AUC. However, the AUC for the above 4 models are similar and are only slightly greater than 0.5, indicating that the response `direction` is hard to be correctly classified with the given predictors.
